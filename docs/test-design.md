...
人が読む形が「どこで途切れたか」を名指しすることは、最終段の行が埋まって
見える構成——同じ起点から複数の枝が出て、一部だけが最終段まで届く——で確かめる。
到達した枝を途切れとして出さないことも同時に見る。片側だけでは、
すべてを途切れとして並べる実装が通ってしまう。
...
人が読む側でも同じ切り分けが要る。標準エラーの語が終了コードと別々に
決まると、不合格と不備が同じ強さで読める。

引数とオプションの誤りは文言の照合で訳しているので、ライブラリが表現を
変えると黙って英語へ戻る。訳せたことは実際にコマンドを走らせて確かめる。
...
- [ ] 標準エラーの語が終了コードの分類と一致し、同じ分類で揺れない
- [ ] 引数とオプションの誤りが日本語で出る
- [ ] OS が返す誤りの訳が、文言ではなく種類で選ばれている
- [ ] 読めない名指しが設定の不備として止まる
...
出力の例も同じである。読み手は例を見て読み方を覚えるので、
形を変えたときに例だけが残ると、案内が実物と食い違う。

ヘルプの枠組みはライブラリが実行時に組み立てるため、コマンドを足したときに
訳し漏れても定義を読むだけでは気づけない。
...
人が読む出力の例は、固定データに対して実際に実行した結果と突き合わせる。
ヘルプは、根とすべての末端サブコマンドについて、ライブラリ由来の英語が
残っていないことを確かめる。対象はどちらもコマンド木から引く。
...
- [ ] 主張された数が実態と一致する
- [ ] 文書に載せた出力の例が実際の出力と一致する
- [ ] すべてのコマンドのヘルプが日本語で揃っている
...
DD-16 の解決規則と配布の設定を検査する。

### 観点
表示の分岐は実行環境に依存する。ldflags の有無と BuildInfo の状態の組み合わせは、
実際の `go install` を単体テストの中で再現できないので、注入した fake で分岐だけを固定する。
未指定の目印が実在する版番号と衝突する誤りは、単体のタスクでは見えず、
Makefile の既定値と合成して初めて現れた種類なので、再発を明示的に固定する。
リリース成果物と `scripts/install.sh` の命名の食い違いは、
タグを push して公開されるまで実挙動として観測できない。

### 検査方法
`cmd/mdtrace/version_test.go` が fake の BuildInfo を注入して優先順位の全分岐を検査し、
Makefile の既定値が明示指定として尊重されることを固定する。
`scripts/install.sh` は CI が構文検査と shellcheck を通す。
タグから公開までの通しは機械で常時は守れないため、リリースのたびに
2 つの導入経路（`scripts/install.sh` と `go install`）の表示がタグと一致することを確認する。

### 合格条件
- [ ] 表示の優先順位の全分岐がテストで固定されている
- [ ] Makefile の既定値を明示した指定が、未指定と誤認されない
- [ ] `scripts/install.sh` が CI の構文検査と shellcheck を通る
- [ ] リリース後、2 つの導入経路の表示がタグと一致する
