...
合格として通ることがない。標準エラーへ出す行の語も同じ分類から引く。
分けた終了コードと語が別々の判断で決まると、両者がずれる。

OS が返す誤りの訳は、文言ではなく**誤りの種類**で判定する。
文言で判定すると、環境や版で表現が変わったときに黙って英語へ戻る。
種類に当てはまらないものは元の文をそのまま通す。
...
欠落の人が読む形は、段ごとの到達済み識別子に加えて、
連鎖の段数より短い経路を「途切れた枝」として名指しする。
経路の長さがそのまま届いた段数なので、欠けている段は経路から求まる。
...
記号と、その意味を並べた凡例は状態の定義から引く。
表示と説明を別々に持つと、状態を足したときに片方だけが取り残される。
...
`cmd/mdtrace/version.go` がバージョン表示文字列を解決し、
...
ヘルプの枠組み（使い方の雛形・`help` コマンド・`-h` と `-v` の説明・
既定値の注記）は cobra と pflag が実行時に組み立てるため、コマンドの
定義側からは訳せない。根でまとめて訳し、コマンド木を歩いて当てる。
一覧を手で持つと、コマンドを足したときに訳し漏れる。
...
オプションの一覧は pflag が組み立てたものの語だけを差し替える。
自前で組み直すと、桁揃えと折り返しの規則を写すことになる。
補完コマンドは説明を差し替える手立てが無いので、一覧から隠すだけにする。
消すと補完そのものが使えなくなる。

引数の個数の検査は自前で持つ。ライブラリの検査は文言が英語で、差し替える
手立てが無い。解析そのものが返す誤り（未知のコマンド・未知のオプション・
値の欠落と変換の失敗）はライブラリの内側で終わるので、文言の照合で訳す。
当てはまらないものは通すので、訳せない誤りが消えることはない。
訳す場所は実行ファイルの入口 1 か所に置く。途中にも置くと、
どちらが効いたのかを追えなくなる。

## DD-16: バージョン解決と配布の設定

BD-17 が定めた表示の優先順位を実装し、配布に関わる設定を束ねる。

### 担当箇所
`cmd/mdtrace/version.go` が表示文字列を解決する。
配布側は `.goreleaser.yaml`（クロスビルドと公開）、
`.github/workflows/ci.yml`（検査）、`.github/workflows/release.yml`（タグ起点の公開）、
`scripts/install.sh`（リリース成果物の取得と配置）が担う。

### 処理
`resolveVersion` は ldflags の値が空でなければそれを使い、
空なら標準ライブラリの `debug.ReadBuildInfo` からモジュールバージョンを取り、
それも取れなければ `dev` を返す。
BuildInfo の取得は変数経由にして、テストが fake を注入できるようにする。

### 留意点
**「未指定」の目印は空文字である。**実在しうるバージョン文字列を目印にすると、
Makefile の既定値と衝突して、明示した指定が黙って無視される（BD-17）。

`scripts/install.sh` はアーカイブ名を版・OS・アーキテクチャから組み立てる。
命名の正典は `.goreleaser.yaml` の name_template で、片方だけ変えると取得が失敗する。
