...
人が読む出力には、この表と同じ意味を凡例として添える。

`gaps` の人が読む出力は、最終段まで届かなかった枝を
「起点 → … → 行き止まり（届かなかった段）」の形で名指しする。
段ごとの到達済み識別子を並べるだけでは、到達した枝と途切れた枝が
同じ段の列に混ざる。最終段の列が埋まって見えるのに警告が付く、
という理由の読めない出力になるためである。
機械可読な出力は経路をそのまま持つので、名指しは人が読む形だけの話である。
...
誤りは標準エラーへ 1 行で出す。行頭の語は終了コードと対応させ、
不合格は「不合格:」、不備は「エラー:」で始める。
どちらも同じ語で始めると、道具が報告した不一致（検証の指摘・辿り切れない
識別子）が、道具が動けなかったことと同じ強さで読める。前者は文書の状態に
ついての事実であって、道具の異常ではない。
...
**利用者向けの出力は日本語で揃える。**説明文だけを訳すと、枠組みが英語のまま
残って出力が 2 か国語に割れる。ライブラリが自前で組み立てる次のものも訳す。

- ヘルプの枠組み（使い方の見出し・オプションの目印・既定値の注記）
- 引数とオプションの解析で返る誤り（未知のコマンド・未知のオプション・
  値の欠落・値の変換の失敗）
- ファイルの有無と権限で OS が返す誤り

訳さないものは 2 種類ある。1 つは機械が読む識別子——コマンド名・
オプション名・出力形式の名前・`--version` の 1 行——で、訳すと
指定できなくなる。もう 1 つは設定の構文解析器が返す位置付きの診断で、
訳し直すと元の文言で調べられなくなる。

訳し分けは持たない。表示する言語は 1 つに固定する（REQ-15 の対象外）。
