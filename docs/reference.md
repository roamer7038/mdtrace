...
凡例: ⚠️ 一部の経路が途中で途切れている / ❌ 下流へ辿る経路が 1 本も無い

⚠️ R-1: ユーザー認証 (docs/requirements.md:3)
   - design: FR-1, FR-2
   - implementation: IMP-1
   途切れ: R-1 → FR-2（implementation が無い）
❌ R-3: 監査ログ (docs/requirements.md:25)
   - design: -
   - implementation: -
   途切れ: R-3（design が無い）
...
`- <段>:` の行は、その起点から届いた識別子を段ごとに並べたものです。
**到達した枝と途切れた枝が同じ行に混ざる**ので、最終段の行が埋まって見えても
最終段まで届いているとは限りません。届いていない枝は `途切れ:` の行が名指しします。
上の例では `FR-1` は `IMP-1` まで届いていますが、`FR-2` に下流がありません。

行き止まりだけを一覧したいときは `--from` で段を下げます。
`mdtrace gaps --from design` なら、行き止まりが `❌ FR-2` の形で行になります。

機械可読な出力（`--format json`）では、各項目の `paths` が経路そのものを持ちます。
連鎖の段数より短い経路が、途切れた枝です。

### `gaps` を門として使うかどうか

**終了コード 1 は「連鎖の終点まで届いていない」という事実の報告です。**
書き漏れの判定ではありません。どちらとして読むかは、連鎖の終点に何を置いたかで決まります。

| 最終段に置いたもの | 読み方 |
|---|---|
| これから書く計画（テスト設計・タスクなど） | 門として使えます。欠落は着手していない上流です |
| 起きたことの記録（試用の所見・障害の記録など） | 一覧として読みます。記録の出ない上流が原理的に残ります |

後者では網羅率 100% を狙って達成するものではありません。
CI で門にするなら `verify` を合否に使い、`gaps` は出力を残すだけにするか、
`--from` で「届いていて当然の段」まで下げて判定します。

この道具自身の文書もこれに当たります。層の構成やライブラリの選定のような
横断的な決定は下流の外部仕様を持たないため、`mdtrace gaps --from architecture`
が欠落として報告しますが、書き漏れではありません（`docs/architecture.md`）。
...
| 症状 | 原因 | 対処 |
|---|---|---|
| 終了コードが 1 か 2 か紛らわしい | 引数や設定の誤りは 2、検証の不合格は 1 | 標準エラーの行頭で見分けます。`不合格:` なら指摘内容を直し、`エラー:` なら設定・引数を見直します |
| `gaps` が ⚠️ を出すのに最終段の識別子は並んでいる | 到達した枝と途切れた枝が同じ行に混ざっています | `途切れ:` の行が行き止まりを名指しします。`--from` で段を下げると行き止まりだけが並びます |
